<template>
    <li >
        ...
        <div>
            <h2>{{user.name}}</h2>
            <h3>
                <span :class="SelectClass('status green','status orange')"></span>
                {{ SelectClass( 'Online', 'Offline' ) }}
            </h3>
        </div>
    </li>
</template>
...
export default {
    ...
    methods : {
        GetOtherUser(){
            axios.get('api/User/'+ this.room.id +'/Room',{
                headers: {
        ...
        listen() {
            Echo.join('chat')
                ...
                .listen('UserOnline', (e) => {
                    ...
                    if( this.user.id === e.user.id )this.user = e.user
                })
